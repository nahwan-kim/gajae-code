...
- `/new`, `fork()`, handoff, `/resume`, and branch transitions now complete verified managed `local://` legacy-root migration before publishing the successor to the agent, workflow-gate emitter, extension hooks, and normal post-transition prompt/path consumers. This matches cold-start `createAgentSession()` (#2797), extends `/resume` (#2925), and prevents the reported immediate post-`/new` failure; atomic visibility of the manager's internal identity is tracked separately in #3138.
