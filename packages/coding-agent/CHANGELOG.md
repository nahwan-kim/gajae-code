...
- `/new`, `fork()`, handoff, and branch transitions now prepare and verify the managed `local://` legacy root before publishing the successor session identity, matching cold-start `createAgentSession()` (#2797) and `/resume` (#2925). Immediate post-transition `local://` resolution no longer fails with "local:// legacy migration must complete before path resolution", while in-place tree navigation keeps its existing session root.
